...
;(function() {
    ...
    function defineProperty(target, name, method, override) {
        ...
        // 不覆盖原有命名空间下的实现
        if(!override && target[name]) return window["console"] && console.log && console.log(target,"target["+name+"] method hava existed,will not overwrite");
     ...
     * @param obj
     */
    function extend(target, obj, override) {
        for (var name in obj) {
            if(obj.hasOwnProperty(name))
                defineProperty.call(target, target, name, obj[name], override);
        }
    }
